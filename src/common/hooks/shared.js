 ...
 * @LastEditTime: 2019-06-01 07:14:27
 * @Description:
 **/
 ...
 */
export function useAbortableFetch(onSuccess = nope, onFailure = nope) {
    const [status, setStatus] = useState(STATUS.INIT);
    const [result, setResult] = useState({});
    if (window.AbortController && !isSafari()) {
        const controller = useRef(new AbortController());
        let abortableFetchRef = useRef(async (url, query) => {
        ...
        let realFetchRef = useRef(async (url, query) => {
            controller.current.abort();
